...
| Library open/recovery | `app.library()` through compatibility lifecycle surface | Transitional | Library lifecycle still needs a focused application owner before removing the compatibility accessor. |
...
| Surface | Current owner call | Adapter-only state | Remaining risk |
| --- | --- | --- | --- |
| Library commands | core library lifecycle compatibility surface | Transitional | Registry and managed-library lifecycle are still coupled in `LocalLibraryService`. |
...
| Library lifecycle, repair, export | direct library compatibility surface | Transitional | These flows need a dedicated lifecycle application owner before the legacy service can stop being runtime-visible. |
...
2. Add use-case owners before migrating tag and library lifecycle commands.
