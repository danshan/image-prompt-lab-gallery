...
| Gallery query/detail/inspector read model | `application::use_cases::albums::QueryGalleryUseCase` | Tauri gallery, daemon task output lookup | Migrated query owner with large legacy adapter | Query behavior enters through application use case. Gallery list/detail/version-tree implementation remains concentrated in `library/gallery.rs` and should continue to split by read-model owner. |
...
4. Continue splitting gallery read-model implementation: version tree, album filter context, asset detail, inspector detail, task origin, and file context remain in `library/gallery.rs`.
