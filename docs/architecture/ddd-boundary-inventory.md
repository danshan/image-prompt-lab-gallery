...
| Gallery query/detail/inspector read model | `application::use_cases::albums::QueryGalleryUseCase` with focused `library::gallery_cards`, `library::gallery_search`, `library::gallery_filtering`, `library::gallery_version_tree`, `library::gallery_detail`, and `library::gallery_task_origin` owners | Tauri gallery, daemon task output lookup | Migrated query owner with split read-model adapters | Query behavior enters through application use case. Gallery card composition, search, filter/album context, version tree, asset detail, inspector detail, reference source, pending suggestion, file context, and task origin read behavior are separated from the gallery adapter. |
