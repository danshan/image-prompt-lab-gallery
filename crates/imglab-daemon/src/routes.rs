...
pub(crate) fn parse_json_body<T: for<'de> Deserialize<'de>>(body: &str) -> DomainResult<T> {
...
pub(crate) fn request_has_valid_token(request: &str, token: &str) -> bool {
...
pub(crate) fn json_response<T: Serialize>(status_code: u16, value: &T) -> HttpResponse {
...
pub(crate) fn error_response(status_code: u16, error: ApiErrorView) -> HttpResponse {
    json_response(status_code, &error)
}

pub(crate) fn domain_error_response(error: DomainError) -> HttpResponse {
...
pub(crate) fn response(status_code: u16, body: &str) -> HttpResponse {
    HttpResponse {
        status_code,
        body: body.to_string(),
    }
...
pub(crate) fn http_response_bytes(response: &HttpResponse) -> String {
...
pub(crate) fn io_error(path: &Path, error: std::io::Error) -> DomainError {
    DomainError::Io {
...
pub(crate) fn serialization_error(error: serde_json::Error) -> DomainError {
