...
use crate::{
    ...
    GallerySort, GenerationEventId, LibraryService, LineageEntry, PendingSuggestionSummaryView,
    PromotedSourceView, ReferenceSourceView, ReviewStatusFilter, SearchQuery, SearchService,
    TaskId, TaskOriginView, TaskStatus, TaskType, VersionSummary, VersionTreeIssue,
    VersionTreeNode,
};
...
impl GalleryReadService for LocalLibraryService {
    ...
    fn get_asset_detail(
        ...
        let promoted_from = current_version
            .as_ref()
            .map(|version| {
                load_promoted_source(&connection, version, &version_tree_model.names_by_id)
            })
            .transpose()?
            .flatten();
...
#[derive(Debug, Default)]
struct VersionTreeModel {
    roots: Vec<VersionTreeNode>,
    names_by_id: HashMap<String, String>,
    issues: Vec<VersionTreeIssue>,
}

#[derive(Debug, Default)]
struct VersionTreeSummary {
    names_by_id: HashMap<String, String>,
    branch_count: u32,
}

fn load_asset_version_tree_summaries(
    connection: &Connection,
...
fn load_asset_scoped_lineage(
    ...
    while let Some(version) = current {
        ...
        let event = version
            .generation_event_id
            .as_ref()
            .map(|event_id| super::assets::load_generation_event(connection, event_id))
            .transpose()?;
        let next = version
            .parent_version_id
            .as_ref()
